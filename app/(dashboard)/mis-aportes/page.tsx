...
export default function MisAportesPage() {
  ...
  useEffect(() => {
    async function loadCampaigns() {
      try {
        const response = await fetch("/api/campanas?available=true", { cache: "no-store" });
  ...
  return <div>
    <div className="mb-6 flex items-start justify-between gap-4">
      <div><h1 className="text-2xl font-extrabold text-ink">Mis aportes</h1><p className="mt-1 text-[13px] text-ink-2">Campañas creadas por otras personas en las que puedes participar.</p></div>
    ...
    {error ? <p className="rounded-lg bg-danger-tint p-4 text-sm text-danger">{error}</p> : visible.length === 0 ? <p className="rounded-lg border border-dashed border-line-2 p-6 text-sm text-ink-2">No hay campañas disponibles para aportar.</p> : <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">{visible.map((campaign) => {
